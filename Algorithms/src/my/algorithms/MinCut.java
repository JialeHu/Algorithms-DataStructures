...
public class MinCut {
	...
	for (int i = 0; i < 100; i++) {
	    HashMap<Integer, List<Integer>> vertices = NumberReader.adjacencyLists(
		    new File("").getAbsolutePath().concat("/src/my/algorithms/testcases/kargerMinCut.txt"));
	...
	while (iterator1.hasNext()) {
	    if (iterator1.next() == target)
		iterator1.remove();
	}
	Iterator<Integer> iterator2 = edges2.iterator();
	while (iterator2.hasNext()) {
	    if (iterator2.next() == vertex)
		iterator2.remove();
	}
