...
import my.data_structures.BinaryTree;
import my.data_structures.BinaryTree.TreeNode;
...
public class HuffmanCoding
{
	...
	public static void main(String[] args) throws FileNotFoundException
	{
		...
		System.out.println("maxLength = maxDepth - 1 = " + (BinaryTree.maxDepth(root) - 1));
		System.out.println("minLength = minDepth - 1 = " + (BinaryTree.minDepth(root) - 1));
		System.out.println("treeSize = " + BinaryTree.sizeOf(root));
		
	}
	
	public static TreeNode huffmanCoding(List<Integer> weights)
	{
		PriorityQueue<Pair<Integer, TreeNode>> heap = new PriorityQueue<>(weights.size(), (p1 ,p2) -> Integer.compare(p1.get1(), p2.get1()));
		weights.stream().forEach( weight -> { heap.offer(new Pair<Integer, TreeNode>(weight, null)); });
		helper(heap);
