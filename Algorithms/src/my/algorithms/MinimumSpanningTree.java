...
public class MinimumSpanningTree {
	...
	// Store edges
	PriorityQueue<Edge> minHeap = new PriorityQueue<Edge>((e1, e2) -> {
	...
	while (spannedNode.size() != graph.getNumOfVertices()) {
		...
		for (Edge edge : edges) {
		    ...
		    if (spannedNode.contains(edge.getHead()) && !spannedNode.contains(edge.getTail())
			    && !visitedEdge.contains(edge)) {
			minHeap.add(edge);
		    } else if (!spannedNode.contains(edge.getHead()) && spannedNode.contains(edge.getTail())
			    && !visitedEdge.contains(edge)) {
			minHeap.add(edge);
		    }
		}
	    ...
	    if (minHeap.isEmpty())
		throw new IllegalArgumentException("No Spanning Tree Found.");
	    ...
	    if (spannedNode.contains(minEdge.getTail()))
		spannedNode.add(minEdge.getHead());
	    else
		spannedNode.add(minEdge.getTail());
	    visitedEdge.add(minEdge);
	}

	// Return minimum spanning edges
