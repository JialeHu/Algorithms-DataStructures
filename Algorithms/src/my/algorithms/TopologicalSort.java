...
public class TopologicalSort {
	...
	HashMap<Integer, List<Integer>> vertices = NumberReader
		.adjacencyLists(new File("").getAbsolutePath().concat("/src/my/algorithms/testcases/kargerMinCut.txt"));
	...
	for (int node : nodes) {
	    if (!visited.get(node))
		DFShelper(graph, node, visited, order);
	}
	return order;
    }
    ...
    private static void DFShelper(HashMap<Integer, List<Integer>> graph, int startNode,
	    HashMap<Integer, Boolean> visited, int[] order) {
	// Mark Visited
