...
/**
 * Maintain middle number and statistic median for a stream of {@code int}
 * input.
 * 
 * @author Jiale Hu
 */
public class MedianMaintainer {
	...
	// Get Test Numbers
	int[] nums = NumberReader
		.int2array(new File("").getAbsolutePath().concat("/src/my/algorithms/testcases/Median.txt"));
	...
	// Check Result
	System.out
		.println("Maintainer Middle: " + mm.getMiddle() + " Array Middle: " + MedianMaintainer.getMiddle(nums));
	System.out
		.println("Maintainer Median: " + mm.getMedian() + " Array Median: " + MedianMaintainer.getMedian(nums));
	...
	Arrays.sort(copy);
	if (len % 2 != 0)
	    return (double) copy[len / 2];
	else
	    return (double) (copy[(len - 1) / 2] + copy[len / 2]) / 2;
    }
	...
	if (isEmpty())
	    throw new EmptyMaintainerException();
    ...
    /**
     * If Maintainer size is even, smaller number is returned and removed. ORDER OF
     * REMOVAL IS NOT GUARANTEED YET.
     * 
     * @return Middle Number of Whole Maintainer
     * @exception EmptyMaintainerException If Maintainer is empty
     */
