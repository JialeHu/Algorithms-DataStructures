...
public class UnionFind<E>
{
	...
	/**
	 * An Graph Edge {@code class} that contains distance between nodes.
	 * Has natural ordering based on ascending distance.
	 * 
	 * @author Jiale Hu
	 *
	...
	/**
	 * @param c a {@code Collection<? extends E>} containing elements to be used in UnionFind
	 */
	public UnionFind(Collection<? extends E> c)
	{
		this();
		for (E e : c) nodes.put(e, new Node<E>(e));
	 ...
	 * @return {@code true} if union is done, {@code false} if union number is below 2 or two elements are identical
	 */
	public boolean union(Edge<E> edge)
	{
		if (numEndNodes < 2) return false;
	...
	{
		int num = 0;
		for (Node<E> node : nodes.values()) if (node.isEnd()) num++;
		return num;
	}
